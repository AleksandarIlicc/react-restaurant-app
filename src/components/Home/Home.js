...
const Home = ({
  ...
  const [establishItem, setEstablishItem] = useState([
    "restaurant",
    "dessert",
    "fastfood",
    "bar",
  ...
  const handleAdd = (value, arr, setArr, checkedFunc) => {
    ...
    const addedRestaurant = allRestaurants.filter(
      (restaurant) => restaurant.type.indexOf(value) >= 0
    );

    const found = arr.some((restaurant) =>
      addedRestaurant.includes(restaurant)
    );
  ...
  const handleDelete = (value, arr, setArr, checkedFunc) => {
    ...
    const deletedRestaurants = arr.filter(
      (restaurant) => restaurant.type.indexOf(value) >= 0
    );
